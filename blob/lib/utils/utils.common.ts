import { generateUuid, isNode, URLBuilder } from "ms-rest-js";
 ...
 * @returns {string}
 */
export function generateBlockID(blockID: number): string {
  const uuid = generateUuid();
  const res = uuid + blockID.toString().padStart(48 - uuid.length, "0");
