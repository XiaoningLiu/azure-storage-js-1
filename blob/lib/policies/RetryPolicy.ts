...
export class RetryPolicy extends BaseRequestPolicy {
  ...
  protected async attemptSendRequest(
    ...
    try {
      this.log(
        ...
        `RetryPolicy: begin ${attempt} attempt(s)`
      );
  ...
  protected shouldRetry(
    ...
    if (attempt >= this.retryOptions.maxTries!) {
      this.log(
        HttpPipelineLogLevel.INFO,
        `RetryPolicy: shouldRetry() attempt(s) ${attempt} >= maxTries ${this
          .retryOptions.maxTries!}, stop retry.`
      );
      return false;
    }
    ...
    if (err) {
      for (const retriableError of retriableErrors) {
        if (
          ...
          this.log(
            HttpPipelineLogLevel.INFO,
            `RetryPolicy: shouldRetry() detects network error ${retriableError} in error name ${
              err.name
            }, error message ${err.message} or error code ${
              err.code
            }, will retry.`
          );
          return true;
