...
describe("PageBlobURL", () => {
  ...
  it("uploadPages", async () => {
    ...
    const page1 = await pageBlobURL.download(Aborter.None, {
      range: { offset: 0, count: 512 }
    });
    const page2 = await pageBlobURL.download(Aborter.None, {
      range: { offset: 512, count: 512 }
    });
  ...
  it("clearPages", async () => {
    await pageBlobURL.create(Aborter.None, 1024);
    let result = await blobURL.download(Aborter.None);
    assert.deepStrictEqual(
    ...
    result = await pageBlobURL.download(Aborter.None, {
      range: { offset: 0, count: 1024 }
    });
