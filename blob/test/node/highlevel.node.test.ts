...
describe("Highelvel", () => {
  ...
  it("UploadFileToBlockBlob should success when blob >= BLOCK_BLOB_MAX_UPLOAD_BLOB_BYTES", async () => {
    ...
    const downloadResponse = await blockBlobURL.download(Aborter.None);
  ...
  it("DownloadBlobToBuffer should success", async () => {
    ...
    await DownloadBlobToBuffer(Aborter.None, blockBlobURL, 0, 0, buf, {
      blockSize: 4 * 1024 * 1024,
      parallelism: 20
    });
  ...
  it("DownloadBlobToBuffer should abort", async () => {
    ...
    try {
      const buf = Buffer.alloc(tempFileLargeLength);
      await DownloadBlobToBuffer(Aborter.timeout(1), blockBlobURL, 0, 0, buf, {
        blockSize: 4 * 1024 * 1024,
        parallelism: 20
      });
      assert.fail();
  ...
  it("DownloadBlobToBuffer should update progress event", async () => {
    ...
    const aborter = Aborter.None;
    try {
      await DownloadBlobToBuffer(aborter, blockBlobURL, 0, 0, buf, {
        blockSize: 1 * 1024,
        parallelism: 1,
        progress: () => {
          eventTriggered = true;
